...
class Juego {
  ...
  nivelSuperado() {
    swal(
      `¡Nivel ${this.nivel - 1} superado!`,
      `Tu puntuación: ${this.puntos}`,
      "./fonts/logo-next_level.png"
  ...
  elegirColor(ev) {
    ...
    if (numeroColor === this.secuencia[this.subNivel]) {
      //verificar si el color elegido es igual al del primer subnivel
      this.subNivel++;
      this.puntos++;
      if (this.subNivel === this.nivel) {
        ...
        if (this.nivel === ultimo_Nivel + 1) {
          //El usuario ha superado el último nivel  ganó
          this.ganoElJuego();
        } else {
      ...
      //el usuario se equivocó de botón, así que pierde
      this.perdioElJuego();
    }
  }
  ganoElJuego() {
    ...
    if (ultimo_Nivel === 5) {
      swal(
        "!Felicitaciones¡ Ganas en modo Fácil, no está mal para practicar.",
        `Tu puntucación: ${this.puntos}`,
        "./fonts/logo-win_the_game.png"
      ).then(this.inicializar);
    } else if (ultimo_Nivel === 10) {
      swal(
        "!Felicitaciones¡ Ganas en modo Normal, ya estás listo/a para ir por un verdadero reto. ¿No te parece?",
        `Tu puntucación: ${this.puntos}`,
        "./fonts/logo-win_the_game.png"
      ).then(this.inicializar);
    } else if (ultimo_Nivel === 15) {
      swal(
        "!Felicitaciones¡ Ganas en modo Medio, te estás volviendo bueno/a en esto. Pero ¿Medio de qué? Mejor sigue avanzando.",
        `Tu puntucación: ${this.puntos}`,
        "./fonts/logo-win_the_game.png"
      );
    } else if (ultimo_Nivel === 25) {
      swal(
        "!Felicitaciones¡ Ganas en modo Difícil, si que estás adquiriendo maestía, que impresionante.",
        `Tu puntucación: ${this.puntos}`,
        "./fonts/logo-win_the_game.png"
      );
    } else if (ultimo_Nivel === 40) {
      swal(
        "!Felicitaciones¡ Ganas en modo Experto, ya eres todo un experto, sin embargo, sigue avanzando y encontrarás sorpresas.",
        `Tu puntucación: ${this.puntos}`,
        "./fonts/logo-win_the_game.png"
      );
    } else if (ultimo_Nivel === 50) {
      swal(
        "!Felicitaciones¡ Ganas en modo Épico. ¿Quién lo pensaría? Te acercas a descubir secretos.",
        `Tu puntucación: ${this.puntos}`,
        "./fonts/logo-win_the_game.png"
      );
    } else if (ultimo_Nivel === 100) {
      swal(
        "!Felicitaciones¡ Ganas en modo Legendario, tu hazaña será recordada en la historia. ¿Estás listo, para conocer el secreto de la inmortalidad?...",
        `Tu puntucación: ${this.puntos}`,
        "./fonts/logo-win_the_game.png"
      );
    } else if (ultimo_Nivel === 999) {
      swal(
        "!Felicitaciones¡ Ganas en modo Superviviencia, no creímos que nadie pudiera jamás. Sin embargo la inmortalidad es algo de gran valor, completa el reto una vez más y se te será revelado el secreto.",
        `Tu puntucación: ${this.puntos}`,
        "./fonts/logo-win_the_game.png"
      );
    } else {
      //el caso de que hubiera un error con el número del último nivel
      swal(
        "!Felicitaciones, ganas el juego¡",
        `Tu puntucación: ${this.puntos}`,
        "./fonts/logo-win_the_game.png"
      ).then(this.inicializar);
    }
  ...
  perdioElJuego() {
    ...
    swal(
      "Lo siento, pierdes esta vez, mejor suerte la próxima",
      `Tu puntucación: ${this.puntos}`,
      "./fonts/logo-game_over.png"
    ).then(() => {
      this.eliminarEventosClick();
      this.inicializar();
    });
